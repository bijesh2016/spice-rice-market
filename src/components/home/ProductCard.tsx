...
export function ProductCard({ product, index = 0 }: ProductCardProps) {
  ...
  // Handle both full Product type and simplified props
  const slug = 'slug' in product && product.slug ? product.slug : product.id;
  const inCart = isInCart(product.id);
  const inWishlist = isInWishlist(product.id);
  
  // Create a minimal product for cart/wishlist if needed
  const productForCart = 'slug' in product ? product : {
    ...product,
    slug: product.id,
      ...
      <Link to={`/product/${slug}`} className="block">
        <div className="relative aspect-square bg-secondary/50 overflow-hidden">
          ...
          <button 
            ...
            onClick={(e) => {
              e.preventDefault();
              toggleWishlist(productForCart as Product);
            }}
          >
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex items-center justify-between gap-2 pt-1">
          ...
          <Button 
            ...
            onClick={() => addToCart(productForCart as Product)}
          >
